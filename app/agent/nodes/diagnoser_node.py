...
def _run_id_from_config(config: Optional[RunnableConfig]) -> Optional[str]:
    ...
    configurable_raw = cfg.get("configurable")
    configurable = configurable_raw if isinstance(configurable_raw, Mapping) else {}
...
#     logger.info(f"evidence_type: {evidence_type}, timeline_events: {timeline_events}, root_cause_hints: {root_cause_hints}, raw: {parsed}")
...
#         else:
#             legacy_action = LEGACY_EVIDENCE_TOOLS.get(msg.name) if isinstance(msg.name, str) else None
#             if legacy_action:
#                 parsed = _safe_json_loads(msg.content)
#                 if parsed is None:
...
#             timeline_candidates,
#             _dict_list(normalized.get("timeline_events")),
#         )
#         root_cause_candidates = _merge_root_cause_candidates(
#             root_cause_candidates,
#             _dict_list(normalized.get("root_cause_hints")),
#         )
